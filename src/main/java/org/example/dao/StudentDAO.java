...
public class StudentDAO {
    ...
    public void addStudent(Student student) {
        try (PreparedStatement preparedStatement = connection.prepareStatement("INSERT into students (group_id, first_name, last_name) values (?,?,?);");) {
    ...
    public void assignCourseToStudentById(int courseId, int studentId) {
        try (PreparedStatement preparedStatement = connection.prepareStatement("insert into students_courses (student_id, course_id) values (?,?)")) {
            preparedStatement.setInt(1, studentId);
    ...
    public void removeStudentFromCourse(int courseId, int studentId) {
        try (PreparedStatement preparedStatement = connection.prepareStatement("delete from students_courses where student_id=? and course_id=?")) {
            preparedStatement.setInt(1, studentId);
            preparedStatement.setInt(2, courseId);
            try {
                preparedStatement.execute();
